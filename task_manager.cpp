...
//const char* stat_name = "RT_GOMP_CLUSTERING_STAT";
...
int main(int argc, char *argv[])
{
	...
	for (unsigned i = 0; i < num_iters; ++i)
	{
		...
		if ( i != 0 ) { //current program has first iteration setup
			...
			//period_timings[i-1] = period_runtime.tv_nsec + nsec_in_sec*period_runtime.tv_sec;
		}
	...
	//temp << std::endl << "Deadlines missed for task " << task_name << ": " << deadlines_missed << " / " << num_iters << std::endl;
	...
	fprintf(stdout,"Avg running time for task %s: %llu  nsec\n", task_name, total_nsec/(num_iters-1));
	...
	for(unsigned i = 0; i < num_iters - 1; i++){
	...
	*/
	...
	//temp.str(std::string());
	//temp << "Max running time for task " << task_name << ": " << max_period_runtime << " secs" << std::endl;
	//tempstr = temp.str();
	//std::cerr << temp;
	
	//temp.str(std::string());
	//temp << std::endl << deadlines_missed << " " << num_iters << " " << omp_get_num_procs() << " " << max_period_runtime << std::endl;
	//tempstr = temp.str();
	//std::cout << temp;
	fflush(stdout);
