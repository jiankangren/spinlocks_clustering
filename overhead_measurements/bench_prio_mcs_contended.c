...
extern void plock_lock(plock *p, mcs_lock *mcs, mcs_lock_t *me);
extern void plock_unlock(plock *p, mcs_lock *mcs, mcs_lock_t *me);
...
void *thr_run(void *input) {
	...
	for (unsigned i=0; i<ITERS_PER_THR; i++) {
		plock_lock(shared_lock, &mcs, &me);
		x++;
		plock_unlock(shared_lock, &mcs, &me);
	}
