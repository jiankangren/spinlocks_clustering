...
extern void plock_lock(plock *p, ticketlock *lock);
extern void plock_unlock(plock *p, ticketlock *lock);
...
void *thr_run(void *input) {
	...
	// Each thread use a distinct local ticket lock
	ticketlock local_lock = {.u = 0};
	...
	for (unsigned i=0; i<ITERS_PER_THR; i++) {
		plock_lock(shared_lock, &local_lock);
		x++;
		plock_unlock(shared_lock, &local_lock);
	}
...
int main(int argc, char *argv[]) {
	...
	plock *shared_lock = (plock*) malloc(sizeof(plock));
	shared_lock->owner = 0;
	shared_lock->waiters = 0;
