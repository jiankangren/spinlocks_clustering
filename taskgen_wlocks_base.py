...
def main_random_base():
	if len(sys.argv) != 6:
		print "Usage: ", sys.argv[0], " <sys_first_core> <sys_last_core> <num_resources> <cs_type> <total_util_frac>"
		exit()
	...
	for cs_num in critical_sections_per_resource:
		...
		#ecrts_folder = 'data_base_ecrts16'
		ecrts_folder = 'tmp'
		directory = ecrts_folder + '/core='+str(m)+'util='+str(fraction)+'resources='+str(num_resources)+'requests='+str(cs_num)+'type='+cs_type
		if os.path.isdir(directory) == False:
			os.system('mkdir ' + directory)
