...
int main(int argc, char *argv[])
{
	...
	std::string locks_name = "/RT_GOMP_CLUSTERING_SPINLOCKS";

	// Name of the shared memory region contains statistics data
	std::string stat_name = "/RT_GOMP_CLUSTERING_STAT";
	...
	//	printf("%s\n%s\n%s\n", barrier_name.c_str(), locks_name.c_str(), stat_name.c_str());
	...
	*/
	...
	// Store children process ids of the forked tasks
	...
	for (unsigned t = 1; t <= num_tasks; ++t)
	{
	    ...
	    )
		{
			...
			// In parent process, store the process id of the child
	...
	while((pid = wait(&status))) {
		if (pid != -1) {
			...
			// If any task return missed deadline, the task set fails to schedule.
			// The clustering launcher will kill the other children since the task set failed.
			if (WEXITSTATUS(status) == RT_GOMP_TASK_MANAGER_MISSED_DEADLINE) {
				schedulable = false;

				for (std::list<pid_t>::iterator it=children.begin(); it!=children.end(); it++) {
					if (*it != pid) {
						// Kill the other children
						int ret = kill(*it, SIGTERM);
						if (ret < 0) {
							printf("WARNING: Send kill signal failed! %s\n", strerror(errno));
						} else {
							printf("Signal SIGTERM sent to process: %d\n", *it);
						}
					}
				}
				// Remove all children id. There is no children anymore.
				children.clear();
			}
		} else {
	...
	// Write the schedulability result of the task set to a file
	std::ostringstream result_file;
	std::string in_file(argv[1]);
	result_file << in_file << ".sched";

	// Write statistics data to a file
	std::ofstream ofs;
	ofs.open(result_file.str().c_str());
	if (!ofs) {
		fprintf(stderr, "ERROR: Schedulability result file open failed.\n");
		return RT_GOMP_CLUSTERING_LAUNCHER_FILE_OPEN_ERROR;
	}
	...
	if (ret_val != SPINLOCKS_UTIL_SUCCESS) {
		fprintf(stderr, "WARNING: Destroy shared memory for spinlocks failed");
	}
	...
	if (!out_file) {
		fprintf(stderr, "WARNING: Statistic file open failed.\n");
	} else {
		for (unsigned i=0; i<resources_number; i++) {
			out_file << (i+1);
	...
	*/
	...
	if (ret_val != STATISTICS_SUCCESS) {
		fprintf(stderr, "WARNING: Destroy shared memory for statistics data failed\n");
	}
	
	fprintf(stderr, "All tasks finished\n");
	return RT_GOMP_CLUSTERING_LAUNCHER_SUCCESS;
}
