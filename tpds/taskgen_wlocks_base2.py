...
def main_random_base():
	if len(sys.argv) != 7:
		print "Usage: ", sys.argv[0], " <sys_first_core> <sys_last_core> <num_tasks> <requests_per_resource> <cs_type> <total_util_frac>"
		exit()
	...
	requests_per_res = int(sys.argv[4])
	cs_type = sys.argv[5]
	fraction = float(sys.argv[6])
	...
	for res_num in num_shared_resources:
		...
		ana_folder = 'data_analysis_tpds/varying_resources'
		directory = ana_folder + '/core='+str(m)+'n='+str(num_tasks)+'util='+str(fraction)+'resources='+str(res_num)+'requests='+str(requests_per_res)+'type='+cs_type
		if os.path.isdir(directory) == False:
			os.system('mkdir ' + directory)
