...
critical_sections_per_resource = [128, 256, 384, 512, 640, 768, 896, 1024]

# Use this for generating task sets with 4 shared resources
#critical_sections_per_resource = [32, 64, 96, 128, 160, 192, 224, 256]
...
def generate_tasks_utils(n, m, fraction):
	lower = 1.25
	upper = math.sqrt(m)
	U = fraction * m
...
def test_tasks_utils():
	for i in range(0, 2):
		utils = generate_tasks_utils(7, 36, 0.75)
		for util in utils:
...
def taskset_generate2(n, m, fraction):
	utils = generate_tasks_utils(n, m, fraction)
	taskset = []
	actual_total_util = 0
	total_util = 0
...
def test_taskset_generate2():
	taskset = taskset_generate2(6, 36, 0.99)
...
# Generate a single base task set. Then for each option of the number of critical sections, 
# create a copy of the base task set and add these critical sections to the new task set. 
# The command line argument for <max_num_requests> is now removed. The number of critical 
# sections per resource is varied from the list critical_sections_per_resource.
def main_random_base():
	
	if len(sys.argv) != 7:
		print "Usage: ", sys.argv[0], " <sys_first_core> <sys_last_core> <num_tasks> <num_resources> <cs_type> <total_util_frac>"
		exit()
	...
	if num_tasks > math.floor(u/1.25) or num_tasks < math.ceil(u/math.sqrt(m)):
	...
	taskset = taskset_generate2(num_tasks, m, fraction)
	...
	# For each value of the number of critical sections per resource, create 
	# a copy of the base task set and distribute the critical sections to it
	for cs_num in critical_sections_per_resource:
		taskset_clone = copy.deepcopy(taskset)
		resources_generate_random(taskset_clone, num_resources, cs_num, cs_type, critsec_lengths)
		distribute_resources_random(taskset_clone, cs_type)
		
		# Write the clone task set to a .rtpt file
		#folder = 'data_analysis_tpds'
		folder = 'data_tpds'

		directory = folder + '/core='+str(m)+'n='+str(num_tasks)+'util='+str(fraction)+'resources='+str(num_resources)+'requests='+str(cs_num)+'type='+cs_type
		if os.path.isdir(directory) == False:
			os.system('mkdir ' + directory)
		write_to_rtpt_random_2(taskset_clone, sys_first_core, sys_last_core, num_resources, directory)
