...
int main(int argc, char *argv[]) {
	...
	bool schedulable = BlockingAnalysis::get_instance().is_schedulable(taskset, num_cores, lock_type);

	if (schedulable == false && taskset->schedulability_status == 0) {
		printf("FATAL ERROR: Schedulability status is 0 while the task is unschedulable!!!\n");
	}
	...
	*/
	...
	//#define __WRITE_RTPS_FILE
	//#ifdef __WRITE_RTPS_FILE
	...
	//#endif // __WRITE_RTPS_FILE

	//	cout << "Last core used: " << current_first_core - 1 << endl;
	
	// For task sets that are deemed schedulable, record the response time
	// bounds of their tasks. This is meant for comparing the bounds with
	// the real response times of the experiments.
	if (taskset->schedulability_status == 0) {
		string resp_time_file = string(argv[1], 0, strlen(argv[1]) - 5);
		resp_time_file += "_rtime_" + string(ltype) + ".txt";
		std::ofstream fs(resp_time_file.c_str());
		if ( !fs.is_open() ) {
			cerr << "ERROR: Cannot open rtps file to write" << endl;
			return -1;
		}		
		...
		for (unsigned i=0; i<num_tasks; i++) {
			unsigned long response_time = BlockingAnalysis::get_instance().response_time(tasks[i+1]->wcet_, 
																	 tasks[i+1]->span_, tasks[i+1]->proc_num_, 
																	 (unsigned long) tasks[i+1]->blocking_whole_job_, 
																	 (unsigned long) tasks[i+1]->blocking_on_single_);

			// Get the min among the computed response time bound and the relative deadline
			response_time = (response_time > tasks[i+1]->period_) ? tasks[i+1]->period_ : response_time;
			//cout << "Response time of task " << (i+1) << ": " << response_time << endl;
			// Format: <task number> <response time>
			fs << (i+1) << " " << response_time << "\n";
		}
